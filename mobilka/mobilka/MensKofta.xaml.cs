...
namespace mobilka
{
    ...
    public partial class MensKofta : ContentPage
    {
        private List<Tovar> _kofta;
        private int userId;
        private int tovarId;
        public MensKofta(int userId)
        {
            InitializeComponent();
            LoadInitialData();
            this.userId = userId;
        ...
        private async void LoadInitialData()
        {
            ...
            _kofta = await repository.GetTovarsAsync();
            _kofta = _kofta.Where(t => t.name.ToLower().Contains("кофта") && t.gender.ToLower() == "мужской").ToList();
            var availableColors = _kofta.Select(t => t.color).Distinct().ToList();
            var availableSizes = _kofta.Select(t => t.size).Distinct().ToList();
            var availableBrands = _kofta.Select(t => t.brend).Distinct().ToList();
            ...
            availableSizes = _kofta.Select(t => t.size).Distinct().ToList();
            DisplayTShirts(_kofta);
        }
        private bool isHeartFilled = false;
        private void DisplayTShirts(IEnumerable<Tovar> tovars)
        {
            koftaStackLayout.Children.Clear();
            foreach (var tovar in tovars)
            {
                ...
                koftaStackLayout.Children.Add(frame);
            }
        }
        ...
        private void ApplyFilters()
        {
            ...
            var filteredTShirts = _kofta.Where(t =>
