...
namespace mobilka
{
    ...
    public partial class MensShtani : ContentPage
    {
        private List<Tovar> _shtani;
        private int userId;
        private int tovarId;
        public MensShtani(int userId)
        {
            InitializeComponent();
            LoadInitialData();
            this.userId = userId;
        ...
        private async void LoadInitialData()
        {
            ...
            _shtani = await repository.GetTovarsAsync();
            _shtani = _shtani.Where(t => t.name.ToLower().Contains("штаны") && t.gender.ToLower() == "мужской").ToList();
            var availableColors = _shtani.Select(t => t.color).Distinct().ToList();
            var availableSizes = _shtani.Select(t => t.size).Distinct().ToList();
            var availableBrands = _shtani.Select(t => t.brend).Distinct().ToList();
            ...
            availableSizes = _shtani.Select(t => t.size).Distinct().ToList();
            DisplayTShirts(_shtani);
        }
        private bool isHeartFilled = false;
        private void DisplayTShirts(IEnumerable<Tovar> tovars)
        {
            shtaniStackLayout.Children.Clear();
            foreach (var tovar in tovars)
            {
                ...
                shtaniStackLayout.Children.Add(frame);
            }
        }
        ...
        private void ApplyFilters()
        {
            ...
            var filteredTShirts = _shtani.Where(t =>
