...
namespace mobilka
{
    ...
    public partial class Platya : ContentPage
    {
        private List<Tovar> platya;
        private int userId;
        private int tovarId;
        public Platya(int userId)
        {
            InitializeComponent();
            LoadInitialData();
            this.userId = userId;
        ...
        private async void LoadInitialData()
        {
            ...
            platya = await repository.GetTovarsAsync();
            platya = platya.Where(t => t.name.ToLower().Contains("платья") && t.gender.ToLower() == "женский").ToList();
            var availableColors = platya.Select(t => t.color).Distinct().ToList();
            var availableSizes = platya.Select(t => t.size).Distinct().ToList();
            var availableBrands = platya.Select(t => t.brend).Distinct().ToList();
            ...
            availableSizes = platya.Select(t => t.size).Distinct().ToList();
            DisplayTShirts(platya);
        }
        private bool isHeartFilled = false;
        private void DisplayTShirts(IEnumerable<Tovar> tovars)
        {
            platyaStackLayout.Children.Clear();
            foreach (var tovar in tovars)
            {
                ...
                platyaStackLayout.Children.Add(frame);
            }
        }
        ...
        private void ApplyFilters()
        {
            ...
            var filteredTShirts = platya.Where(t =>
