...
namespace mobilka
{
    ...
    public partial class Zhenskie_bruki : ContentPage
    {
        private List<Tovar> bruki_zh;
        private int userId;
        private int tovarId;
        public Zhenskie_bruki(int userId)
        {
            InitializeComponent();
            LoadInitialData();
            this.userId = userId;
        ...
        private async void LoadInitialData()
        {
            ...
            bruki_zh = await repository.GetTovarsAsync();
            bruki_zh = bruki_zh.Where(t => t.name.ToLower().Contains("брюки") && t.gender.ToLower() == "женский").ToList();
            var availableColors = bruki_zh.Select(t => t.color).Distinct().ToList();
            var availableSizes = bruki_zh.Select(t => t.size).Distinct().ToList();
            var availableBrands = bruki_zh.Select(t => t.brend).Distinct().ToList();
            ...
            availableSizes = bruki_zh.Select(t => t.size).Distinct().ToList();
            DisplayTShirts(bruki_zh);
        }
        private bool isHeartFilled = false;
        private void DisplayTShirts(IEnumerable<Tovar> tovars)
        {
            bruki_zhStackLayout.Children.Clear();
            foreach (var tovar in tovars)
            {
                ...
                bruki_zhStackLayout.Children.Add(frame);
            }
        }
        ...
        private void ApplyFilters()
        {
            ...
            var filteredTShirts = bruki_zh.Where(t =>
