...
namespace mobilka
{
    ...
    public partial class MensKurtki : ContentPage
    {
        private List<Tovar> _kurtki;
        private int userId;
        private int tovarId;
        public MensKurtki(int userId)
        {
            InitializeComponent();
            LoadInitialData();
            this.userId = userId;
        ...
        private async void LoadInitialData()
        {
            ...
            _kurtki = await repository.GetTovarsAsync();
            _kurtki = _kurtki.Where(t => t.name.ToLower().Contains("куртка") && t.gender.ToLower() == "мужской").ToList();
            var availableColors = _kurtki.Select(t => t.color).Distinct().ToList();
            var availableSizes = _kurtki.Select(t => t.size).Distinct().ToList();
            var availableBrands = _kurtki.Select(t => t.brend).Distinct().ToList();
            ...
            availableSizes = _kurtki.Select(t => t.size).Distinct().ToList();
            DisplayTShirts(_kurtki);
        }
        private bool isHeartFilled = false;
        private void DisplayTShirts(IEnumerable<Tovar> tovars)
        {
            kurtkiStackLayout.Children.Clear();
            foreach (var tovar in tovars)
            {
                ...
                kurtkiStackLayout.Children.Add(frame);
            }
        }
        ...
        private void ApplyFilters()
        {
            ...
            var filteredTShirts = _kurtki.Where(t =>
