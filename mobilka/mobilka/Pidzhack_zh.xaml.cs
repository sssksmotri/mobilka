...
namespace mobilka
{
    ...
    public partial class Pidzhack_zh : ContentPage
    {
        private List<Tovar> pidzchak;
        private int userId;
        private int tovarId;
        public Pidzhack_zh(int userId)
        {
            InitializeComponent();
            LoadInitialData();
            this.userId = userId;
         ...
         private async void LoadInitialData()
        {
            ...
            pidzchak = await repository.GetTovarsAsync();
            pidzchak = pidzchak.Where(t => t.name.ToLower().Contains("пиджаки") && t.gender.ToLower() == "женский").ToList();
            var availableColors = pidzchak.Select(t => t.color).Distinct().ToList();
            var availableSizes = pidzchak.Select(t => t.size).Distinct().ToList();
            var availableBrands = pidzchak.Select(t => t.brend).Distinct().ToList();
            ...
            availableSizes = pidzchak.Select(t => t.size).Distinct().ToList();
            DisplayTShirts(pidzchak);
        }
        private bool isHeartFilled = false;
        private void DisplayTShirts(IEnumerable<Tovar> tovars)
        {
            pidzchakStackLayout.Children.Clear();
            foreach (var tovar in tovars)
            {
                ...
                pidzchakStackLayout.Children.Add(frame);
            }
        }
        ...
        private void ApplyFilters()
        {
            ...
            var filteredTShirts = pidzchak.Where(t =>
